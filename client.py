...
from models import Paraphrased, Detected
...
class Paraphraser(object):
    ...
    def _paraphrase(self, text, dest, src):
    ...
    def paraphrase(self, text, dest='en', src='auto'):
        ...
        if isinstance(text, list):
            result = []
            for item in text:
                paraphrased = self.paraphrase(item, dest=dest, src=src)
                result.append(paraphrased)
            return result

        origin = text
        data = self._paraphrase(text, dest, src)

        # this code will be updated when the format is changed.
        paraphrased = ''.join([d[0] if d[0] else '' for d in data[0]])
        ...
        if not PY3:  # pragma: nocover
            ...
            if isinstance(paraphrased, str):
                paraphrased = paraphrased.decode('utf-8')

        result = Paraphrased(src=src, dest=dest, origin=origin,
                            text=paraphrased, pronunciation=pron, extra_data=extra_data)
    ...
    def detect(self, text):
        ...
        data = self._paraphrase(text, dest='en', src='auto')
